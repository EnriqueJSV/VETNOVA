...
AS BEGIN
	BEGIN TRY
		...
		INNER JOIN Veterinarios VET ON VET.Id_Veterinario = CIT.Id_Veterinario
		WHERE
			MASC.Nombre LIKE '%' + @Filtro + '%'
			OR PROP.Nombre LIKE '%' + @Filtro + '%'
			OR PROP.Apellido1 LIKE '%' + @Filtro + '%'
			OR VET.Nombre LIKE '%' + @Filtro + '%'
			OR VET.Apellido1 LIKE '%' + @Filtro + '%'
			OR CIT.Motivo LIKE '%' + @Filtro + '%'
			OR CIT.Estado_Cita LIKE '%' + @Filtro + '%'
		ORDER BY CIT.Fecha DESC, CIT.Hora DESC
	END TRY
	BEGIN CATCH
		SELECT CAST(NULL AS INT) AS Id_Cita, CAST(NULL AS VARCHAR(100)) AS Mascota, CAST(NULL AS VARCHAR(200)) AS Propietario,
